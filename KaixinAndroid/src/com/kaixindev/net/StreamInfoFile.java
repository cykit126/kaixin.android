...
public class StreamInfoFile {
     ...
     * Methods
     */
    public StreamInfoFile(RandomAccessFile file) {
        mFile = file;
    }
     ...
     */
    public static StreamInfoFile newInstance(String path) {
        if (path != null && path.length() > 0) {
            try {
                RandomAccessFile file = new RandomAccessFile(path,"rw");
                return new StreamInfoFile(file);
            }
...
//                e.printStackTrace();
     ...
     * @return Returns null if failed.
     */
    public static StreamInfoFile newInstance(File file) {
        if (file != null) {
     ...
     * @return Returns true if success, otherwise returns false.
     */
    public boolean setCurrentSize(final int which, final long size)
    {
        try {
            final long oldOffset = mFile.getFilePointer();
            if (which % 2 == 0) {
     ...
     * @return Returns true if success, otherwise returns false.
     */
    public boolean setLastModified(final long size) {
        return writeSize(mFile, OFFSET_LAST_MODIFIED, size);
    }
